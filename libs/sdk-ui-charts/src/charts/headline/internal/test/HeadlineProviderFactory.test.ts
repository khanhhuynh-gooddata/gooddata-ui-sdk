...
describe("HeadlineProviderFactory", () => {
    describe("createHeadlineProvider", () => {
        const createBuckets = (primaryMeasure: IMeasure = null, secondaryMeasures: IMeasure[] = []): IBucket[] => {
            return [
        ...
        it("should create MultiMeasureProvider in case 1 primaryMeasure and multiple secondaryMeasures has provided", () => {
            const buckets = createBuckets(ReferenceMd.Amount, [ReferenceMd.Amount_1.Sum, ReferenceMd.Amount_1.Avg]);
            const config = createConfig(true);
